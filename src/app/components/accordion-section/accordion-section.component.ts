...
export class AccordionSectionComponent implements OnInit, OnChanges {
  ...
  // No usamos @Input para userId, sino que lo tomamos de la ruta
  userId: number = 1;  // Valor por defecto
  @Input('id') id?: number;
  ...
  plant: Plants = {
    ...
    intervale_fumig: 0,
    savePlant: true,
    userId: this.userId,  // Asignamos el userId obtenido de la ruta
  };
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe((params) => {
      this.userId = +params.get('userId')!;  // Convertir a número
      this.plant.userId = this.userId;  // Asignar el userId al objeto planta
      console.log('UserId obtenido de la ruta:', this.userId);

      // Llamar a getById con el userId
      if (this.id) {
  ...
  addEventFert() {
    for (let index = 0; index < 3; index++) {
      ...
      this.eventService.create(this.userId, this.eventData).subscribe();  // Usamos userId en la creación del evento
    }
    this.showDialog();
  }
