...
export class DetailsComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    if (this.plantId) {
      this.plantsService.getById(this.plantId).subscribe((response: Plants) => {
        this.plant = response;  // La respuesta ahora se ajusta automáticamente al tipo Plants
      });
    }
  }
  ...
  showDialog() {
    const dialogRef = this.dialog.open(DialogSavePlantComponent, {
      width: '450px',
      data: {
        id: this.plant?.id,  // Usamos el operador de encadenamiento opcional para evitar errores si plant es undefined
      },
    });
